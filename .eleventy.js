...
/* 网站内容数据 */
const lxfeiDataDir = "notes/_data";
let dataDir = lxfeiDataDir;
let distPath = "_site";

const categories = require("./" + dataDir + "/categories.json");
...
module.exports = function(eleventyConfig) {
  ...
  /**
   * How to use: 
   * - Markdown Highlighter (triple backtick ```js)
   * - Liquid Custom Tag `{% highlight js%}`
   * - Numjunks Paired Shortcode `{% highlight js %}`
   * - JavaScript Function `this.highlight("js", code)`
   */
  ...
  eleventyConfig.addPlugin(require("./src/_11ty/optimize-html"));
  ...
  eleventyConfig.addFilter("htmlDateString", (dateObj) => {
    return DateTime.fromJSDate(dateObj, { zone: "utc+8" });
  });
  ...
  eleventyConfig.addFilter("getRandomColor", function (name, _postIdx, idx) {
    ...
    // Based on index -> new posts make old posts' color change
    // function getHash(input, numColors) {
 ...
 // 有问题
  ...
  // Get the first `n` elements of a colletion
  ...
  eleventyConfig.addPairedShortcode("hsbox", (content, title) => {
    return (
      '<div class="hsbox"><div class="hs_title">' +
  ...
  eleventyConfig.addShortcode("ref", function (url) {
    return (
      '<sup><a href="' +
      url +
      '" rel="noopener noreferrer" target="_blank">[ref]</a>'
    );
  });
